...
int rgb15_dma_pio = dma_claim_unused_channel(true);
int rgb15_dma_cb = dma_claim_unused_channel(true);
...
void initVGA() {
  ...
  rgb15_program_init(pio, rgb15_sm, rgb15_offset);
  //pio_sm_init(pio, sm, offset, &c);
}
void rgb15_program_init(PIO pio, uint sm, uint offset) {
  pio_sm_config c = rgb15_program_get_default_config(offset);

  // Configure 32-bit out for scattered pins (GPIO0-4, 6-10, 11-15)
  sm_config_set_out_pins(&c, 0, 32);
  pio_sm_set_consecutive_pindirs(pio, sm, 0, 16, true);

  for (int i = 0; i <= 15; i++) {
    if (i == 5) continue;  // GPIO5 not used
    pio_gpio_init(pio, i);
...
dma_channel_config c0 = dma_channel_get_default_config(rgb15_dma_pio);  // default configs
channel_config_set_transfer_data_size(&c0, DMA_SIZE_8);                 // 8-bit txfers
channel_config_set_read_increment(&c0, true);                           // yes read incrementing
channel_config_set_write_increment(&c0, false);                         // no write incrementing
channel_config_set_dreq(&c0, DREQ_PIO0_TX2);                            // DREQ_PIO0_TX2 pacing (FIFO)
channel_config_set_chain_to(&c0, rgb15_dma_cb);                         // chain to other channel

dma_channel_configure(
  rgb15_dma_pio,        // Channel to be configured
  &c0,                  // The configuration we just created
  &pio->txf[rgb15_sm],  // write address (rgb15 PIO TX FIFO)
...
dma_channel_config c1 = dma_channel_get_default_config(rgb15_dma_cb);  // default configs
channel_config_set_transfer_data_size(&c1, DMA_SIZE_32);               // 32-bit txfers
channel_config_set_read_increment(&c1, false);                         // no read incrementing
channel_config_set_write_increment(&c1, false);                        // no write incrementing
channel_config_set_chain_to(&c1, rgb15_dma_pio);                       // chain to other channel

dma_channel_configure(
  rgb15_dma_cb,                          // Channel to be configured
  &c1,                                   // The configuration we just created
  &dma_hw->ch[rgb15_dma_pio].read_addr,  // Write address (channel 0 read address)
...
dma_channel_set_irq0_enabled(rgb15_dma_pio, true);
// set DMA IRQ handler
...
// Note that the rgb15 state machine is running at full speed,
...
// of that array.
dma_start_channel_mask((1u << rgb15_dma_pio));
}
